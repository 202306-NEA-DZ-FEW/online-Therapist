...
function Button({
    ...
    if (color.toLocaleLowerCase() === "teal" && !disabled) {
        return transition ? (
            ...
            >
                <span className={`absolute inset-0 flex items-center justify-center w-full h-full text-white duration-300 ${rotate? 'translate-x-full': '-translate-x-full'} bg-Teal group-hover:translate-x-0 ease`}>
                    {buttonHoverText ? (
                        buttonHoverText
                    ) : (
                        <svg
                ...
                <span className={`absolute flex items-center justify-center w-full h-full text-Teal transition-all duration-300 transform ${rotate? 'group-hover:-translate-x-full': 'group-hover:translate-x-full'} ease`}>
                    {buttonText}
                </span>
